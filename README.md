...
- **Alpha Vantage Integration**: ✅ **NEW - Comprehensive API coverage**

### 🎯 **ALPHA VANTAGE INTEGRATION HIGHLIGHTS**
- **📊 API Coverage**: 70+ endpoints (stocks, forex, crypto, commodities, technical indicators)
- **🏢 Company Coverage**: Top 200 global companies by market cap
- **🗄️ Enhanced Schema**: 16 specialized tables for all data types
- **🤖 Vector Integration**: Semantic search for investment research
- **⚡ Rate Management**: Intelligent rate limiting for free & premium tiers
...
├── backend/
│   ├── db/
│   │   ├── postgres_handler.py        # psycopg2 / async
│   │   ├── redis_cache.py
│   │   ├── migrations/
│   │   │   ├── 001_init.sql
│   │   │   ├── 002_add_price_gaps.sql
│   │   │   └── ...
│   │   ├── queries/
│   │   │   ├── common_queries.py      # assets, prices, news
│   │   │   ├── options_queries.py     # Member-1
│   │   │   ├── move_queries.py        # Member-2
│   │   │   ├── macro_queries.py       # Member-3
│   │   │   └── utils.py
│   │   └── seed/
│   │       └── seed_assets.sql
│
└── vector_db/
 ...
 Clean separation
  You provide DB layer once → all members query it
...
## ✅ Critical Requirements When Populating PostgreSQL & VectorDB

These are the 12 key rules that guarantee strong RAG & ML performance.

### ✅ 1. Structure All Data to be Queryable by:

✅ **ticker**  
✅ **timestamp**  
✅ **risk type** (sentiment, regulatory, infra, options, macro)  
✅ **source**

You already implemented this — very good.
This allows:

✔ time-window filtering (move explainer)  
✔ asset-specific retrieval (AAPL vs BTC)  
✔ risk-aware retrieval (macro vs sentiment vs options)

📌 **Without this structure**: RAG becomes noisy and inaccurate.

### ✅ 2. Deduplicate Before Insert

RAG hates duplicates — it pollutes embeddings and lowers relevance.

✅ **For market_prices use**:
```sql
ON CONFLICT (ticker, timestamp) DO NOTHING;
```

✅ **For vectorDB, generate deterministic chunk IDs**:
```
...
✅ **150–350 characters**  
✅ **OR 2–4 sentences each**

**Why?**
- Too small → embeddings lose meaning
- Too large → irrelevant garbage gets retrieved and confuses LLM

So your preprocessing must:
✔ remove URLs, boilerplate  
✔ compress noise ("Advertisement", "Read more", repeated tickers)

### ✅ 4. Store Metadata With Every Vector Chunk
...
✅ **Without this metadata, RAG is blind.**

### ✅ 5. Normalize Text Before Embedding
...
✅ remove emojis  
✅ collapse whitespace  
✅ decode weird unicode

Your `preprocess_pipeline.py` is already doing this — good.

### ✅ 6. ONLY Embed Meaningful Information
...
✅ **Embed only** cleaned text + summary + metadata.

### ✅ 7. Coherent Timestamps
...
### ✅ 9. Embed Summaries, Not Entire Documents

Perplexity, SEC filings, RBI press releases - they are long.

✅ **First summarize → then chunk → embed**
...
### ✅ 10. Backfill at Least 30–90 Days of Data

ML models (anomalies, macro forecast, sentiment) require history.

**Minimum**:
✅ 30 days of `market_prices`  
✅ 14 days of `news_headlines`  
✅ 30 days of `regulatory_events`

Without this → ML scores and RAG context become weak.

### ✅ 11. Consistent Severity Scale
...
✔ RAG uses severity to rank evidence  
✔ alerts rely on consistent severity tags

### ✅ 12. ML Outputs Must Also Feed the VectorDB

This is where most teams fail.

**After anomaly detector runs**:
✅ store anomaly explanation chunks in vectorDB with:
```
risk_type = "anomaly"
severity = high
...
✅ store model-generated summaries too

**Why?**
Because RAG gains intelligence from ML outputs, not only raw text.

### ✅ What Happens If You Follow All These Rules

✅ Member-1 options interpreter finds actual call/put evidence  
✅ Member-2 sudden move explainer can tie price action to events  
✅ Member-3 macro gap forecaster sees Fed/RBI evidence + sentiment  
✅ Chat endpoint produces data-backed answers  
✅ Alerts become accurate and trustable
